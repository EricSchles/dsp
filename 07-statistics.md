...
![image of Baye's Theorem](http://www.life.umd.edu/labs/delwiche/MSyst/lec/figures/BayesTheorem.jpg)

For this problem, the conditional property of the problem allows us to use Baye's Theorem to determine the probability that Elvis was an identical twin. A few assumptions will have to be made though. **A'** stands for **not A**, which I'll dive into later.

**Assumptions**:
- Assume that equal number of boys and girls are born in average
- P(F) = Fraternal twin = 1/125
- P(I) = Identical twin = 1/300
- P(T) = Twins

**Breakdown**:

I first determined the probability that fraternal twins are both boys using the following calculation. Since I assumed that both boys and girls are born evenly, I went ahead and used 1/2 for both independent events:
 > P(T|F) = 1/2 × 1/2 = 1/4

I then calculated the probability that identical twins are both boys, which equates to:
...
> probability of identical twins given that they're twins = P(I|T)

Which then equals:

> P(T|I)P(I) / P(T)

The bottom half of P(T|I)P(I) / P(T) can then be rewritten (since this is asking for the probability of twins):
